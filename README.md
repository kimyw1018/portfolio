...
## ✨ 주요 기능
- **다크 모드 & 라이트 모드**: 사용자의 OS 환경에 맞춘 자동 전환 및 수동 토글 지원 (설정 브라우저 저장)
- **Zero-Build Architecture**: CDN 기반 Tailwind CSS + Devicon + Lucide Icon 적용으로 빌드 오류나 의존성 충돌 없음
- **프로젝트 필터링 & 상세 팝업**: Backend / Frontend 카테고리 필터 및 클릭 시 상세 모달 제공
- **원클릭 이메일 복사**: 버튼 클릭 시 토스트 메시지와 함께 클립보드에 이메일 자동 복사
- **반응형 디자인**: 모바일, 태블릿, 데스크톱 전 기기 최적화 레이아웃
